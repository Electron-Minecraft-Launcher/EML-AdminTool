...
$get_user = $db->prepare('SELECT * FROM users WHERE id = ? AND accepted = 3 AND server_admin = 0');
$get_user->execute([$_GET['user']]);

if ($user = $get_user->fetch()) {
	$delete_user = $db->prepare('DELETE FROM users WHERE id = ? AND accepted = 3 AND server_admin = 0');
