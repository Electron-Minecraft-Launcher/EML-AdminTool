...
$get_user = $db->prepare('SELECT * FROM users WHERE id = ? AND accepted = 1 AND server_admin = 0');
$get_user->execute([$_POST['user']]);

if ($user = $get_user->fetch()) {
	$accept_user = $db->prepare('UPDATE users SET p_files_updater = ?, p_bootstrap = ?, p_maintenance = ?, p_send_news = ?, p_edit_del_news = ?, p_background = ? WHERE id = ? AND accepted = 1 AND server_admin = 0');
